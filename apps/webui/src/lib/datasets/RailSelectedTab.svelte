<script lang="ts">
	...
	interface Props {
		...
		onSetMaterialPreviewValue: (v: string) => void;
...
<section class="rail-section rail-tool-panel">
	...
	{#if inspectorTab === 'object'}
		<label>
			<span>label</span>
			<input value={item.label ?? ''} oninput={(e) => onUpdateField('label', (e.currentTarget as HTMLInputElement).value)} />
		</label>
		{#if itemKind === 'object'}
		...
		<button class="button button-subtle full danger" onclick={onDelete}>Delete {itemId}</button>
	{:else}
