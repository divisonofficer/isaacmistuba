<script lang="ts">
	import { buildGenericJobRows, jobStageLabel, jobStatusClass, normalizeJobStatus } from '$lib/datasets/batchHelpers';
...
{:else}
	<div class="empty">No render jobs in this batch.</div>
{/if}
...
<style>
	...
	.generic-row {
		display: grid;
		grid-template-columns: minmax(150px, 1.2fr) minmax(120px, 1fr) minmax(140px, 1fr) 96px minmax(120px, 1fr);
		gap: var(--space-2);
		align-items: center;
		padding: 7px var(--space-3);
		font-size: var(--font-size-xs);
	}
	.generic-head { position: sticky; top: 0; z-index: 1; background: var(--surface-2); color: var(--muted-strong); font-weight: 700; }
	.generic-row {
		border: 0;
	...
	.generic-row span { min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
	.js-chip { justify-self: start; padding: 1px 6px; border-radius: 99px; font-size: 10px; font-weight: 700; }
