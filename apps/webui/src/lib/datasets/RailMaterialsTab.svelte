<script lang="ts">
	import { getObjectMaterialView } from '$lib/api';
	import MaterialInspector from '$lib/datasets/MaterialInspector.svelte';
	import { opticalIorLabel, polarimetricBrdfLabel, rgbCss } from '$lib/datasets/materialHelpers';

	interface Props {
		projectId: string | null | undefined;
		sceneId: string | null | undefined;
	}
	let { projectId, sceneId }: Props = $props();

	let payload = $state<any>(null);
	let loading = $state(false);
	let error = $state('');
	let search = $state('');
	let usedOnly = $state(true);
	let selectedId = $state<string | null>(null);
	let expanded = $state<Record<string, boolean>>({});
	let reqSeq = 0;

	$effect(() => {
		const pid = projectId;
		const sid = sceneId;
		if (!pid || !sid) {
			payload = null;
			return;
		}
		const seq = ++reqSeq;
		loading = true;
		error = '';
		getObjectMaterialView(pid, sid, {})
			.then((res) => {
				if (seq !== reqSeq) return;
				payload = res;
				loading = false;
			})
			.catch((err) => {
				if (seq !== reqSeq) return;
				error = String(err?.message ?? err);
				payload = null;
				loading = false;
			});
	});

	const materials = $derived((payload?.materials ?? []) as any[]);
	const summary = $derived(payload?.summary ?? null);

	// Infinigen assigns per-instance shader copies (shader_bone.001, .002, …) that
	// share the same params. Group by base name so 1267 rows collapse into families.
	function baseName(id: string): string {
		return String(id ?? '')
			.replace(/^shader_/, '')
			.replace(/\.\d+$/, '');
	}
	function variantSuffix(id: string): string {
		const m = String(id ?? '').match(/\.(\d+)$/);
		return m ? `.${m[1]}` : '·';
	}

	const groups = $derived.by(() => {
		let items = materials;
		if (usedOnly) items = items.filter((m) => (m.used_by_count ?? 0) > 0);
		const q = search.trim().toLowerCase();
		if (q)
			items = items.filter((m) =>
				`${m.material_id} ${m.material?.optical_class ?? ''} ${m.material?.bsdf_strategy ?? ''}`
					.toLowerCase()
					.includes(q),
			);
		const map = new Map<string, { name: string; items: any[]; used: number }>();
		for (const m of items) {
			const b = baseName(m.material_id);
			let g = map.get(b);
			if (!g) {
				g = { name: b, items: [], used: 0 };
				map.set(b, g);
			}
			g.items.push(m);
			if ((m.used_by_count ?? 0) > 0) g.used += 1;
		}
		return [...map.values()].sort(
			(a, b) => b.items.length - a.items.length || a.name.localeCompare(b.name),
		);
	});

	const shownCount = $derived(groups.reduce((n, g) => n + g.items.length, 0));
	const selectedRow = $derived(materials.find((m) => m.material_id === selectedId) ?? null);
	const selectedResolved = $derived(
		selectedRow
			? { ...selectedRow, bsdf_mode: payload?.bsdf_mode, injection_enabled: payload?.injection_enabled }
			: null,
	);

	function toggle(name: string) {
		expanded = { ...expanded, [name]: !expanded[name] };
	}
</script>

<section class="rail-section rail-tool-panel">
	<div class="rail-title">Materials</div>
	{#if summary}
		<div class="mt-sub">
			{summary.objects} objects · <strong>{summary.referenced}</strong>/{summary.materials} used
			{#if summary.orphan}· {summary.orphan} unused{/if}
			{#if payload?.bsdf_mode}<span class="mt-mode">mode: {payload.bsdf_mode}</span>{/if}
		</div>
	{/if}

	{#if error}
		<div class="mt-error">failed to load materials: {error}</div>
	{:else if loading && !payload}
		<div class="mt-muted">loading scene materials…</div>
	{:else if materials.length === 0}
		<div class="mt-muted">No materials in this scene's authoring map.</div>
	{:else}
		<div class="mt-controls">
			<input
				class="mt-search"
				placeholder="filter…"
				value={search}
				oninput={(e) => (search = (e.currentTarget as HTMLInputElement).value)}
			/>
			<label class="mt-toggle">
				<input type="checkbox" checked={usedOnly} onchange={(e) => (usedOnly = (e.currentTarget as HTMLInputElement).checked)} />
				used only
			</label>
		</div>
		<div class="mt-count">{groups.length} families · {shownCount} materials</div>

		<div class="mt-list">
			{#each groups as g (g.name)}
				{#if g.items.length === 1}
					{@const m = g.items[0]}
					{@const sw = rgbCss(m.material?.base_color)}
					<button
						class="mt-row"
						class:active={m.material_id === selectedId}
						onclick={() => (selectedId = m.material_id)}
					>
						<span class="mt-sw" style={sw ? `background:${sw}` : ''}></span>
						<span class="mt-name">{g.name}</span>
						<span class="mt-tags">
							<span class="mt-chip">{polarimetricBrdfLabel(m.material?.bsdf_strategy)}</span>
							{#if (m.used_by_count ?? 0) > 0}
								<span class="mt-used" title="objects using this material">{m.used_by_count}</span>
							{:else}
								<span class="mt-orphan">unused</span>
							{/if}
						</span>
					</button>
				{:else}
					<div class="mt-group">
						<button class="mt-ghead" onclick={() => toggle(g.name)}>
							<span class="mt-caret">{expanded[g.name] ? '▾' : '▸'}</span>
							<span class="mt-name">{g.name}</span>
							<span class="mt-tags">
								<span class="mt-gcount">×{g.items.length}</span>
								{#if g.used > 0}<span class="mt-used">{g.used} used</span>{/if}
							</span>
						</button>
						{#if expanded[g.name]}
							{#each g.items as m (m.material_id)}
								{@const sw = rgbCss(m.material?.base_color)}
								<button
									class="mt-row mt-variant"
									class:active={m.material_id === selectedId}
									onclick={() => (selectedId = m.material_id)}
								>
									<span class="mt-sw" style={sw ? `background:${sw}` : ''}></span>
									<span class="mt-name mt-vsuffix">{variantSuffix(m.material_id)}</span>
									<span class="mt-tags">
										<span class="mt-ior">{opticalIorLabel(m.optical_resolution)}</span>
										{#if (m.used_by_count ?? 0) > 0}
											<span class="mt-used">{m.used_by_count}</span>
										{:else}
											<span class="mt-orphan">unused</span>
										{/if}
									</span>
								</button>
							{/each}
						{/if}
					</div>
				{/if}
			{/each}
		</div>

		{#if selectedResolved}
			<MaterialInspector {projectId} {sceneId} materialId={selectedId} resolved={selectedResolved} />
		{/if}
	{/if}
</section>

<style>
	.mt-sub {
		font-size: 11.5px;
		color: var(--muted, #667085);
		margin-top: 2px;
		display: flex;
		gap: 6px;
		flex-wrap: wrap;
		align-items: baseline;
	}
	.mt-mode {
		font-family: ui-monospace, Menlo, Consolas, monospace;
	}
	.mt-controls {
		display: flex;
		gap: 8px;
		align-items: center;
		margin-top: 10px;
	}
	.mt-search {
		flex: 1 1 auto;
		padding: 6px 8px;
		border: 1px solid var(--line, #d9dee8);
		border-radius: 6px;
		font-size: 12.5px;
	}
	.mt-toggle {
		display: flex;
		align-items: center;
		gap: 4px;
		font-size: 11.5px;
		color: var(--muted, #667085);
		white-space: nowrap;
	}
	.mt-count {
		font-size: 11px;
		color: var(--muted, #667085);
		margin: 6px 0 4px;
	}
	.mt-list {
		display: flex;
		flex-direction: column;
		gap: 2px;
		max-height: 34vh;
		overflow-y: auto;
		margin-bottom: 10px;
	}
	.mt-group {
		display: flex;
		flex-direction: column;
	}
	.mt-ghead,
	.mt-row {
		display: flex;
		align-items: center;
		gap: 8px;
		width: 100%;
		padding: 5px 7px;
		border: 1px solid transparent;
		border-radius: 6px;
		background: transparent;
		cursor: pointer;
		text-align: left;
		font-size: 12.5px;
	}
	.mt-ghead:hover,
	.mt-row:hover {
		background: #f3f6fb;
	}
	.mt-row.active {
		background: #eef2ff;
		border-color: #c7d2fe;
	}
	.mt-variant {
		padding-left: 22px;
	}
	.mt-caret {
		width: 12px;
		color: var(--muted, #667085);
		flex: 0 0 auto;
	}
	.mt-sw {
		width: 16px;
		height: 16px;
		border-radius: 4px;
		border: 1px solid var(--line, #d9dee8);
		flex: 0 0 auto;
		background: repeating-conic-gradient(#eee 0% 25%, #fff 0% 50%) 50% / 8px 8px;
	}
	.mt-name {
		flex: 1 1 auto;
		white-space: nowrap;
		overflow: hidden;
		text-overflow: ellipsis;
	}
	.mt-vsuffix {
		font-family: ui-monospace, Menlo, Consolas, monospace;
		color: var(--muted, #667085);
	}
	.mt-tags {
		display: flex;
		align-items: center;
		gap: 6px;
		flex: 0 0 auto;
	}
	.mt-chip {
		padding: 1px 6px;
		border-radius: 999px;
		background: #eef2ff;
		color: #3538cd;
		font-size: 10.5px;
	}
	.mt-gcount {
		font-size: 10.5px;
		color: var(--muted, #667085);
		font-family: ui-monospace, Menlo, Consolas, monospace;
	}
	.mt-used {
		padding: 0 6px;
		border-radius: 999px;
		background: #ecfdf3;
		color: #067647;
		font-size: 10.5px;
	}
	.mt-orphan {
		font-size: 10.5px;
		color: #98a2b3;
	}
	.mt-ior {
		font-size: 10.5px;
		color: var(--muted, #667085);
		font-family: ui-monospace, Menlo, Consolas, monospace;
	}
	.mt-muted,
	.mt-error {
		font-size: 12px;
		color: var(--muted, #667085);
		margin-top: 8px;
	}
	.mt-error {
		color: #b42318;
	}
</style>
