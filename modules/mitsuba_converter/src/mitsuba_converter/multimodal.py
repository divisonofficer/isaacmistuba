...
MODALITY_DEFINITIONS = {
    ...
    "lidar_point_cloud": "3D spinning LiDAR point cloud generated by Mitsuba scene ray intersections.",
...
def _render_lidar_point_cloud(
    ...
    *,
    ...
    origin, right, up, forward = _camera_basis(camera_to_world)
    horizontal_samples = max(1, int(config.width))
    vertical_channels = max(1, int(config.height))
    h_fov = float(config.lidar_horizontal_fov_deg)
    h_angles = np.deg2rad(np.linspace(-h_fov * 0.5, h_fov * 0.5, horizontal_samples, endpoint=False, dtype=np.float32))
    v_angles = np.deg2rad(
        np.linspace(
            float(config.lidar_vertical_fov_min_deg),
            float(config.lidar_vertical_fov_max_deg),
            vertical_channels,
            dtype=np.float32,
        )
    )
    min_range = float(config.lidar_min_range_m)
    max_range = float(config.lidar_max_range_m)

    points = np.full((vertical_channels, horizontal_samples, 3), np.nan, dtype=np.float32)
    normals = np.full_like(points, np.nan)
    directions = np.zeros_like(points)
    ranges = np.full((vertical_channels, horizontal_samples), np.nan, dtype=np.float32)
    valid = np.zeros((vertical_channels, horizontal_samples), dtype=bool)
    intensity = np.zeros((vertical_channels, horizontal_samples), dtype=np.float32)

    render_start = time.perf_counter()
    for vi, v_angle in enumerate(v_angles):
        cv = float(np.cos(v_angle))
        sv = float(np.sin(v_angle))
        for hi, h_angle in enumerate(h_angles):
            direction = _normalize(
                forward * (cv * float(np.cos(h_angle)))
                + right * (cv * float(np.sin(h_angle)))
                + up * sv
            )
            directions[vi, hi] = direction.astype(np.float32)
            try:
                ray = mi.Ray3f(
                ...
                is_valid = bool(si.is_valid())
                hit_range = float(si.t) if is_valid else float("nan")
            except Exception:
                is_valid = False
                hit_range = float("nan")
                si = None
            if not is_valid or not np.isfinite(hit_range) or hit_range < min_range or hit_range > max_range:
                continue
            hit_point = origin + direction * hit_range
            normal = np.array([float(si.n[0]), float(si.n[1]), float(si.n[2])], dtype=np.float32) if si is not None else np.zeros(3, dtype=np.float32)
            points[vi, hi] = hit_point.astype(np.float32)
            normals[vi, hi] = normal
            ranges[vi, hi] = float(hit_range)
            valid[vi, hi] = True
            incidence = max(0.0, float(np.dot(-direction, _normalize(normal))) if np.linalg.norm(normal) > 1e-8 else 0.0)
            intensity[vi, hi] = float(incidence / max(hit_range * hit_range, 1e-6))

    render_s = time.perf_counter() - render_start
    total_s = time.perf_counter() - start
    point_cloud = np.concatenate([points.reshape(-1, 3), ranges.reshape(-1, 1)], axis=1).astype(np.float32)
    result = ModalityResult(
        ...
        metadata={
            "sensor_model": "spinning_lidar_scene_intersection",
            "horizontal_samples": horizontal_samples,
            "vertical_channels": vertical_channels,
            "horizontal_fov_deg": h_fov,
            "vertical_fov_deg": [float(config.lidar_vertical_fov_min_deg), float(config.lidar_vertical_fov_max_deg)],
            "range_m": [min_range, max_range],
            "wavelength_nm": float(config.lidar_wavelength_nm),
...
def _needs_lidar(modalities: set[str]) -> bool:
    return "lidar_point_cloud" in modalities
