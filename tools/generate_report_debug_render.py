    ...
    for vp in d.get("viewpoints", []):
        ...
        for s in vp["sensors"]:
            ...
            blocks += (f'<h3>{s["sensor_id"]} <span class=mut>· {s["sensor_type"]} · fov {s["fov_h_deg"]:.0f}° · '
                       f'mount {s["mount"]} · modalities {s["modalities"]}</span></h3>{head}{rows}')
...
S1/S0·S2/S0·S3/S0 <span class=sw style="background:linear-gradient(90deg,#3b4cbf,#fff,#bf2626)"></span> −1…+1</p>
...
<p class=mut>재현: <code>tools/debug_render_rig.py --rig {d['rig']}</code> → <code>tools/generate_report_debug_render.py</code> ·
