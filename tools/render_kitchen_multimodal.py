...
def make_pseudo_nir_maps(xml_path: Path, out_dir: Path) -> dict[str, str]:
...
def write_mapviz_xml(xml_path: Path, out_path: Path, channel: str) -> Path | None:
    """Map-visualization scene: every shape becomes a flat `diffuse` whose reflectance
    is its baked `channel` map (base_color/roughness/normal/metallic), lit by a single
    uniform white constant emitter (radiance 1) so the render ≈ the map value itself.
    (Self-emitting area lights fail on all-zero maps — no importance-sampling mass —
    so a diffuse+uniform readout is used instead.) Shapes lacking the map read black.
    Returns None if no material carries this channel."""
    suffix = {"albedo": "_base_color.png", "roughness": "_roughness.png",
              "normal": "_normal.png", "metallic": "_metallic.png"}[channel]
    tree = ET.parse(xml_path)
    root = tree.getroot()
    bsdf_tex: dict[str, str] = {}
    for b in root.findall("bsdf"):
        ...
        for s in b.iter("string"):
            if s.get("name") == "filename" and str(s.get("value", "")).endswith(suffix):
                bsdf_tex[bid] = s.get("value"); break
    for em in list(root.findall("emitter")):
        root.remove(em)
    for b in list(root.findall("bsdf")):
        root.remove(b)
    ET.SubElement(root, "emitter", type="constant").append(
        ET.fromstring('<rgb name="radiance" value="1 1 1"/>'))
    n = 0
    for shape in root.findall("shape"):
        ref = shape.find("ref")
        fn = bsdf_tex.get(ref.get("id")) if ref is not None else None
        if ref is not None:
            shape.remove(ref)
        bsdf = ET.SubElement(shape, "bsdf", type="diffuse")
        if fn is None:
            ET.SubElement(bsdf, "rgb", attrib={"name": "reflectance", "value": "0 0 0"})
            continue
        tex = ET.SubElement(bsdf, "texture", attrib={"name": "reflectance", "type": "bitmap"})
        ET.SubElement(tex, "string", attrib={"name": "filename", "value": fn})
        if channel in ("roughness", "normal", "metallic"):
...
def main() -> int:
    ...
    manifest = {"scene_id": a.scene_id, "views": [], "groups": a.groups,
                "spp": a.spp, "size": [a.width, a.height]}
    for vi, vp in enumerate(vps):
        ...
        rec = {"index": vi, "node_id": nid, "position": node["position"], "images": {}}
        ...
        if "active_nir" in a.groups:
            r = render_group(pseudo_xml, cam, a.fov, ["active_nir_intensity"], cfg, assist_flat, tex_cap=256)
        ...
        if "mapviz" in a.groups:
            for ch, mode in (("normal", "gray"), ("roughness", "gray"), ("metallic", "gray")):
                ...
                if vx is None:
                    continue
                r = render_group(vx, cam, a.fov, ["rgb"], cfg, tex_cap=None)
                save_png(r["rgb"].array, out / f"vp{vi}_{nid}_map_{ch}.png", "gray")
